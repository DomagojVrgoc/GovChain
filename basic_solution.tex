...
\node (d1) [below left  = 0.8 and 0.25 of ghost1]{$d_1$};
...
\path (d1) edge node {} (ghost1);
\path (d2) edge node {} (ghost1);
\path (d3) edge node {} (ghost2);
\path (d4) edge node {} (ghost2);



\end{tikzpicture}
\caption{Each day, a Merkle tree is constructed using that day's published documents $d_1,d_2,\dots$. The root of this tree is then hashed along with the hash value of the previous day, and the output value is broadcast along with the published documents. The result is a monitorable hash list $[x_1,x_2,\dots]$.}
